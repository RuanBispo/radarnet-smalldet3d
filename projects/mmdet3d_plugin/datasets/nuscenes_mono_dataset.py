...
def nusc_box_to_cam_box3d(boxes):
    ...
    """
    locs = torch.as_tensor([b.center for b in boxes]).view(-1, 3)
    dims = torch.as_tensor([b.wlh for b in boxes]).view(-1, 3)
    rots = torch.as_tensor([b.orientation.yaw_pitch_roll[0] for b in boxes]).view(-1, 1)
    velocity = torch.as_tensor([b.velocity[:2] for b in boxes]).view(-1, 2)
